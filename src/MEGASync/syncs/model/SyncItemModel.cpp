...
QVariant SyncItemModel::data(const QModelIndex& index, int role) const
{
    ...
    switch (index.column())
    {
        case Column::ENABLED:
        {
            ...
            {
                return sync->getRunState() == ::mega::MegaSync::RUNSTATE_RUNNING ? Qt::Checked :
                                                                                   Qt::Unchecked;
            }
            break;
        }
...
QVariant SyncItemModel::getColumnStats(
    int role,
    mega::MegaHandle backupId,
    std::function<int(const ::mega::MegaSyncStats&)> statsGetter) const
{
    if (role == Qt::DisplayRole)
    {
...
SyncItemSortModel::SyncItemSortModel(QObject* parent):
...
bool SyncItemSortModel::lessThan(const QModelIndex& source_left,
